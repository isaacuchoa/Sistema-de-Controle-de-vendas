...
********************************************************************/
if($_POST["txtLocal"] == "formCadCliente"){
	//captando dados do Formulário
	
	$nome = $_POST["noCliente"];
	$endereco = $_POST["enCliente"];
	$telefone = $_POST["teCliente"];
	$email = $_POST["emCliente"];
	$cidade = $_POST["ciCliente"];
	$estado = $_POST["esCliente"];
	$cep = $_POST["ceCliente"];
	$cEmpresa = $_POST["cliEmpresa"];
	
		
	//instanciando o Objeto de Cadastro
	$cad = new ManipulateData();
	$cad->setTable("Cliente");
	
	$cad->setFields("nomeCliente, enderecoCliente, telefoneCliente, emailCliente, cidadeCliente, estadoCliente, cepCliente, idEmpresa");
	$cad->setDados("'$nome', '$endereco', '$telefone', '$email', '$cidade', '$estado', '$cep', '$cEmpresa'");
	$cad->insert();
	$erro = base64_encode($cad->getStatus());
	
@header("Location: ../?telas=frmCadCliente&msn=$erro");
}
